...
import json, os
from app.sqlite_config import init_db, _conn
...
ADVANCED_SUGGESTION_DATA = {
    ...
    "installation_onboarding": {
        ...
        "action_steps": [
            ...
            "Simplify onboarding to 2–3 essential steps",
...
def populate():
    ...
    cur = conn.cursor()
    # settings
    cur.execute("REPLACE INTO settings(k,v) VALUES(?,?)", ("TOP_N_ISSUES", str(TOP_N_ISSUES)))
    # issue_clusters
    for k,v in ISSUE_CLUSTERS.items():
        cur.execute("REPLACE INTO issue_clusters(cluster,keywords) VALUES(?,?)", (k, json.dumps(v)))
    for a,kws in ASPECT_KEYWORDS.items():
        cur.execute("REPLACE INTO aspect_keywords(aspect,keywords) VALUES(?,?)", (a, json.dumps(kws)))
    for a,txt in SUGGESTION_MAP.items():
        cur.execute("REPLACE INTO suggestion_map(aspect,suggestion) VALUES(?,?)", (a, txt))
    for a,data in ADVANCED_SUGGESTION_DATA.items():
        cur.execute("REPLACE INTO advanced_suggestions(aspect,data) VALUES(?,?)", (a, json.dumps(data)))
    conn.commit()
    conn.close()
    print("DB populated.")
